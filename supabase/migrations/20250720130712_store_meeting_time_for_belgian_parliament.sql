...
drop function if exists "public"."get_meetings_by_filter"(source_tables text[], source_ids text[], max_results integer, start_date timestamp with time zone, end_date timestamp with time zone, country text);
drop function if exists "public"."get_meetings_by_filter"(source_tables text[], source_ids text[], max_results integer, start_date timestamp with time zone, end_date timestamp with time zone, country text, topics text[]);
...
alter table "public"."belgian_parliament_meetings" alter column "meeting_date" set data type timestamp without time zone using "meeting_date"::timestamp without time zone;
...
AS $function$
    SELECT vm.*
    FROM v_meetings vm
    WHERE (source_tables IS NULL OR vm.source_table = ANY(source_tables))
      AND (source_ids IS NULL OR vm.source_id = ANY(source_ids))
      AND (start_date IS NULL OR vm.meeting_start_datetime >= start_date)
      AND (end_date IS NULL OR vm.meeting_start_datetime <= end_date)
      AND (country IS NULL OR vm.location = country)
      AND (topics IS NULL OR vm.topic = ANY(topics))
    ORDER BY vm.meeting_start_datetime DESC
    LIMIT max_results;
$function$;

CREATE OR REPLACE FUNCTION public.search_meeting_suggestions(
    search_term text,
    max_results integer DEFAULT 10
)
 RETURNS TABLE(meeting_id text, title text, source_table text, relevance numeric)
 LANGUAGE sql
AS $function$
    SELECT 
        vm.meeting_id,
        vm.title,
        vm.source_table,
        similarity(vm.title, search_term) as relevance
    FROM v_meetings vm
    WHERE vm.title ILIKE '%' || search_term || '%'
    ORDER BY relevance DESC
    LIMIT max_results;
$function$;

CREATE OR REPLACE FUNCTION public.get_meeting_source_tables()
 RETURNS TABLE(source_table text)
 LANGUAGE sql
AS $function$
    -- Description: returns all distinct source_table names in v_meetings
    SELECT DISTINCT vm.source_table
    FROM public.v_meetings vm
    ORDER BY vm.source_table;
$function$;
